...
RetryPolicy = _reflection.GeneratedProtocolMessageType(
    ...
    (_message.Message,),
    {
        ...
        "__module__": "google.cloud.pubsub_v1.proto.pubsub_pb2",
        "__doc__": """A policy that specifies how Cloud Pub/Sub retries message
  delivery.
...
DeadLetterPolicy = _reflection.GeneratedProtocolMessageType(
  ...
  Attributes:
      dead_letter_topic:
          ...
          subscription’s parent project (i.e., service-{project_number
          }@gcp-sa-pubsub.iam.gserviceaccount.com) must have permission
          to Publish() to this topic.  The operation will fail if the
          topic does not exist. Users should ensure that there is a
...
PushConfig = _reflection.GeneratedProtocolMessageType(
    ...
    Attributes:
        service_account_email:
            ...
            /service-accounts>`__ to be used for generating the OIDC
            token. The caller (for CreateSubscription, UpdateSubscription,
            and ModifyPushConfig RPCs) must have the
...
ReceivedMessage = _reflection.GeneratedProtocolMessageType(
  ...
  Attributes:
      ...
      delivery_attempt:
          ...
          and number of ack_deadline exceeds) for this message.  A NACK
          is any call to ModifyAckDeadline with a 0 deadline. An
          ack_deadline exceeds event is whenever a message is not
